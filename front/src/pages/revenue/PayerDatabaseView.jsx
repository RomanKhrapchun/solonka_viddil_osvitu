...
const PayerDatabaseView = () => {
    const { id } = useParams();
    const navigate = useNavigate();
    const { error, status, data } = useFetch(`api/revenue/payer-database/${id}`);
        ...
        if (status === STATUS.SUCCESS && data) {
            return {
                columns: [
                    ...
                    {
                        title: 'ID типу платника',
                        dataIndex: 'payer_type_id',
                    },
                    {
                        title: 'Тип платника',
                        dataIndex: 'type_name',
            ...
            {status === STATUS.SUCCESS ? (
                <React.Fragment>
                    <div className="btn-group" style={{ marginBottom: '10px' }}>
                        <Button icon={onBackIcon} onClick={() => navigate('/revenue/payer-database')}>
                            Повернутись до бази платників
                        </Button>
                        <Button type="primary" icon={onEditIcon} onClick={() => navigate(`/revenue/payer-database/${id}/edit`)}>
                            Редагувати
                        </Button>
...
export default PayerDatabaseView;
